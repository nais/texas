...
impl Token {
    fn new(claims: TokenClaims) -> Self {
        Self {
            ...
            claims,
            signing_key: SIGNING_KEY.parse::<jwk::JsonWebKey>().unwrap().key.to_encoding_key(),
        }
    }
    ...
    fn encode(&self) -> String {
        jwt::encode(&self.header, &self.claims, &self.signing_key).unwrap()
    }
}
